"""
This module provides methods for parsing and indexing a UCSC refGene file.
...
fields = {1:"name", 2:"chrom", 3:"strand", 4:"txStart", 5:"txEnd", 6:"cdsStart", 7:"cdsEnd", 8:"exonCount", 9:"exonStarts", 10:"exonEnds", 12:"alias"}

def parse(infile):
    """Parses a UCSC refGene file"""
    txts = []
    ff = open(infile, 'r')  
...
def parse_line(line):
    """Parses an individual record of a UCSC refGene file"""
    cols = line.rstrip("\n").split("\t")
    if cols[0]:
...
def txt2gene(infile):
    ...
    for line in open(infile, 'r'):
        count += 1
        cols = line.rstrip('\n').split('\t')
        if not genes.has_key(cols[12]):
            genes[cols[12]] = []
        genes[cols[12]].append([cols[1], count])
